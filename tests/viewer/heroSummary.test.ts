...
// 검증 대상 규칙 ↔ 시나리오:
//  - viewer-readability 불변 "상세 화면 제목 아래 요약이 두 가지 이상의 이야기를 담고 있으면,
//    각 이야기는 저마다 독립된 줄을 차지한다"
//    → 문장이 끝나는 자리마다 항목을 나눈다 / 여러 항목이면 목록으로 그린다 / 한 덩어리 문단으로 그리지 않는다
//  - viewer-readability 불변 "요약을 끊어 보여줄 때 원래 글의 내용은 하나도 더해지거나 빠지지
//    않는다 — 나누는 자리만 정할 뿐이다" → 나뉜 항목을 도로 이으면 원문과 같다
//  - viewer-readability 불변 "한 가지 이야기만 담긴 요약은 끊지 않고 그대로 한 줄로 보여준다"
//    → 문장이 하나뿐이면 나누지 않는다 / 항목이 하나면 문단(p)을 유지한다 / 빈 요약은 아무것도 그리지 않는다
//  - viewer-readability 허용 "뜻이 마무리되는 자리에서 요약을 나누어 항목마다 줄을 바꾸고, 항목
