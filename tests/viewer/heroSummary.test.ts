// @concept:detail-summary-outline
// tests/viewer/heroSummary.test.ts
// 상세 화면 제목 아래 요약과 허용·제한 카드(assets/viewer.js)의 표시를 node:vm + 최소 DOM 스텁으로 검증한다.
// 검증 대상 규칙(detail-summary-outline):
//  - 불변1 "두 가지 이상의 이야기를 담고 있으면 각 이야기는 저마다 독립된 줄을 차지한다"
//    → 요약이 여러 항목이면 항목마다 별도 요소(li)로 나뉜다. (restrict "한 덩어리 글로 이어 붙이는 것" 금지)
//  - 불변2 "원래 글의 내용은 하나도 더해지거나 빠지지 않는다"
//    → 나뉜 항목을 도로 이으면 원문과 같다. (restrict "없던 말을 지어 넣거나 있던 말을 빼는 것" 금지)
//  - 불변3 "한 가지 이야기만 담긴 요약은 끊지 않고 그대로 한 줄로 보여준다"
//    → 항목이 하나면 문단(p)을 유지한다. (allow "나눌 자리가 없으면 한 줄로 두는 것")
//  - restrict3 "이름·번호·자릿수처럼 붙어 있어야 뜻이 통하는 표기를 도중에 자르는 것"
//    → 경로·판번호 안의 마침표는 끊는 자리가 아니다.
//  - allow2 "항목마다 앞에 표식을 붙여 항목의 시작을 알리는 것"
//    → 표식을 붙일 수 있도록 목록(ul.hero__points) 구조로 그린다.
...
// 검증 대상 규칙(detail-summary-outline 맞물림): "요약 아래로 이어지는 본문 각 절은 원래 글의
