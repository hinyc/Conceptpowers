...
// 검증 대상 규칙 ↔ 시나리오:
//  - detail-summary-outline 불변 "상세 화면 제목 아래 요약이 두 가지 이상의 이야기를 담고 있으면,
//    각 이야기는 저마다 독립된 줄을 차지한다"
//    → 문장이 끝나는 자리마다 항목을 나눈다 / 여러 항목이면 목록으로 그린다 / 한 덩어리 문단으로 그리지 않는다
//  - detail-summary-outline 불변 "요약을 끊어 보여줄 때 원래 글의 내용은 하나도 더해지거나 빠지지
//    않는다 — 나누는 자리만 정할 뿐이다" → 나뉜 항목을 도로 이으면 원문과 같다
//  - detail-summary-outline 불변 "한 가지 이야기만 담긴 요약은 끊지 않고 그대로 한 줄로 보여준다"
//    → 문장이 하나뿐이면 나누지 않는다 / 항목이 하나면 문단(p)을 유지한다 / 빈 요약은 아무것도 그리지 않는다
//  - detail-summary-outline 허용 "뜻이 마무리되는 자리에서 요약을 나누어 항목마다 줄을 바꾸고, 항목
//    앞에 표식을 붙이는 것" → 표식을 붙일 수 있도록 목록(ul.hero__points) 구조로 그린다
//  - detail-summary-outline 제한 "이름·번호·자릿수처럼 붙어 있어야 뜻이 통하는 표기를 끊는 자리로
//    오해해 도중에 자르는 것" → 파일 이름·판번호 안의 마침표는 끊는 자리가 아니다
//  - detail-summary-outline 맞물림 "요약 아래로 이어지는 본문 각 절은 원래 글의 생김새를 그대로
