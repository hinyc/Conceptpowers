// @concept:viewer-navigation @concept:list-filter @concept:output-locale
...
// 검증 대상 규칙 ↔ 시나리오:
//  - viewer-navigation 허용 "사람이 아무 선택도 한 적 없을 때만 기본값을 쓰는 것"
//    → 아무 선택도 한 적 없으면 기본 열림이다 (곁 목록은 넓은 화면 전용이라 너비는 보지 않는다)
//  - viewer-navigation 불변 "기록된 선택이 있으면 기본값보다 그 선택을 우선한다"
//    → 사용자가 닫으면 닫힘을 기억해 유지한다 / 닫았다가 다시 열면 열림을 기억해 유지한다
//  - viewer-navigation 불변 "선택을 기록하는 것은 사람이 직접 조작했을 때뿐이다 — 상황 변화로는
//    기록하지 않는다"
//    → 저장 경로를 타는 것은 setOpen(사람의 조작)뿐이다
//  - list-filter 불변 "좁히기는 이미 떠 있는 목록 안에서만 걸러낸다 — 목록에 없는 것을 새로
//    데려오지 않는다" → matchesQuery: 빈 검색어는 항상 true / 대소문자 무시 부분일치 /
