// @concept:atomic-baseline-write @concept:init-gate
// 정렬 기록 폴더의 저장소 제외 설정을 검증한다.
// 검증 대상 규칙 ↔ 시나리오:
//  - atomic-baseline-write 구성요소 "대상: 개념·기능 본문과 도구가 관리하는 상태 기록(기준선, 변경
//    이력, 충돌 기록, 검사 증빙, 마지막 커밋 표시)"
//    → .alignment/.gitignore를 만들어 마지막 커밋 표시(last-commit)를 무시 목록에 넣는다
//      (기록 가운데 공유할 것과 각자 것을 가른다)
//  - atomic-baseline-write 불변 "저장 도중 실패하면 … 실패를 감추지 않는다"와 같은 태도로,
