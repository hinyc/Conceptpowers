...
export const checkUnapprovedRed: GateCheck = async ({ report }) => {
  ...
  return {
    ...
    context:
      'Commit gate (D17): For the staged changes, confirm you ran conceptpowers:review (code↔concept) and, when concepts changed, the consistency check of conceptpowers:update-concepts (concept↔concept). Some referenced concepts are still red (unapproved) — surface this prominently and let the user decide whether to commit.',
  };
};
