...
async function renderViewerToDisk(root, stampVersion) {
  ...
  await copyAsset("topnav.js", join5(p.conceptsViewer, "assets", "topnav.js"));
...
async function loadAliases(root, home = homedir3()) {
...
async function readReferenceLock(root) {
  try {
    const raw = JSON.parse(await readFile12(cpPaths(root).referenceLock, "utf8"));
...
async function buildSessionStartOutput(root, pluginRoot, deps = {}) {
  ...
  const context = [
    ...
    "- Concept rules are requirements, not observations: a green concept states what MUST hold, and the current code may not comply yet. Before telling the user how the system behaves, confirm it in the code; when code and a concept disagree, report it as a possible violation (conceptpowers:review) instead of presenting the rule as current fact.",
    `- Where to look first: start from concepts for intent, rules spread across files, and "is X allowed / why is it like this" questions (viewer manifest index, then 1-3 concept files); go straight to the code for a single value, one file's implementation detail, UI specifics, or "where/how is X implemented" \u2014 concepts do not say where or how something is implemented, and anything outside every concept's scope is answered from the code.`,
    "- What the commit gate does NOT check: it never judges whether code obeys a concept, changed or not. For a changed concept it only checks that related code (or a no-code record) and the evidence ride in the same commit; beyond that it checks tags, unapproved/conflicted concepts, quality floors, test follow-up and scope, reference confidentiality and governance files; whether code follows a rule is judged only when conceptpowers:review runs \u2014 so run it for behavior changes.",
    ...
    `- Concept status: green(settled contract: user-authored, consistency-checked, user-confirmed)/pending(user-authored, awaiting settle)/red(auto-inferred or rejected). The agent may only promote a user-authored pending to green after a passing, attested consistency check and the user's confirmation; it must NEVER demote or change a settled green/red \u2014 the user does that directly. Never auto-approve a red (un-authored) concept.`,
    ...conceptTestsLine,
    ...enforcementLine,
