...
- **상호작용:** 노드를 드래그하면 위치가 고정되고, *개념*·*기능* 노드를 클릭하면 상세로 이동한다(`#/concept/:slug`, `#/feature/:slug`). *파일* 노드는 상세 페이지가 없는 말단이라, hover 시 **전체 경로와 경로 복사 버튼**을 툴팁으로 보여준다(노드 위 라벨은 파일명만 줄여 표시).
