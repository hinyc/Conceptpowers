...
| `conceptDrivenTests` | `true` \| `false` (boolean만, 문자열은 검증 실패)                                                                                              | `true`                                      | 테스트도 개념의 지배를 받는다 — 켜져 있으면 세션 시작 규칙에 "테스트 작성·수정 전 대상 코드의 개념을 찾아 allow/restrict/immutableRules에서 시나리오를 도출하라"가 주입된다. `false`로 명시하면 꺼진다.                                                                                |
