...
| 개념이 바뀌었는데 **딸린 테스트가 이번 커밋에 하나도 없음**(검토 기록도 없음)       | **ask**   | `[TEST REVIEW] …` — 개념에 맞춰 테스트를 고쳐 함께 스테이징하거나, 고칠 필요가 없으면 `attest-test-review <slug> --result no-impact\|no-tests --note "<사유>"`로 기록                                                              |
| 스테이징된 **테스트 파일이 가리키는 개념이 없음**(마커 없음 또는 `@concept:none`)   | **ask**   | `[TEST SCOPE] …` — 첫머리에 `@concept:<slug>`를 적는다. 일반 코드와 달리 테스트에는 `@concept:none`을 쓸 수 없다                                                                                                                   |
