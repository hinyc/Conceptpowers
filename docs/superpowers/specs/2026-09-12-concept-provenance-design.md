...
- 규칙 뼈대
  - 모든 개념은 자기 근거를 하나 이상 밝힌다
  - 근거에는 원문을 옮겨 적지 않고 위치만 적는다
  - 근거만 고치는 것은 개념 수정이 아니다
  - 근거는 코드 판단의 입력이 아니다
...
1. **근거 칸 도입** — 스키마(A) + 뷰어(C) + 상태 예외(D) + 스킬(E) + 새 개념(G) + 테스트.
   품질 결격 검사는 아직 켜지 않는다.
2. **백필 + 결격 전환** — 23개 백필(F) + 품질 결격(B) + 테스트. 같은 커밋이라 게이트가 볼 때는
   이미 전부 채워져 있다.
...
| 상태 전이      | `sources`만 고치면 green 유지, 다른 칸이 섞이면 pending 강등           |
