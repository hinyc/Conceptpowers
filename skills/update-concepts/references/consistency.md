...
3. Apply the **status-aware resolution rule** when a conflict is found:
   - **green is the settled contract.** When a newly approved (green) concept conflicts with a `red`
   ...
   - Pending(🟡) concepts are user-authored drafts under check. On a clean result, the caller settles them to green after the user confirms; on a conflict, they stay pending and the reason is recorded via
...
- `review` (an LLM judgment) covers code changes and this procedure covers concept changes; the commit gate only
  checks that their records exist, not that they were thorough.
- **Unapproved (red) concepts follow the enforcement level**: `strict` denies the commit, `standard` asks, `light`
  warns. When you see the warning, show it prominently; never approve a red concept to get past it. Approval happens
  only on the user's explicit request (the `update-concepts` approve flow D).
