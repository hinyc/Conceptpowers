...
- 이것이 **개념의 상위 기준**(높은 층이 낮은 층의 개념을 제약)임을 한 줄로 설명하고,
  지금 작성할지 / 건너뛸지 묻는다.
- 작성한다면 사용자가 내용을 말하고 에이전트는 `conceptpowers:update-baseline` 절차로
  받아 적는다(사용자 명시 요청이 이 스킬의 질문·답으로 성립한다). **에이전트가 내용을
  지어내 채우지 않는다.**
- 건너뛰면 "baseline 없이 정의된 개념은 나중에 상위 기준과 어긋날 수 있다"고 한 줄
  경고하고 다음 단계로.
