...
| `conceptpowers:define-feature`    | **When you surface a feature** (button / action / route / command) that should appear in the viewer's feature index.            | A feature JSON under `features/` — its `concepts` power the chips on the feature's home index row (and a concept detail's "related features"), and its `codePaths` feed the commit gate's drift-follow judgment.                                                                                  |
...
How the links are made: the `concept → file` edges come from the concept's own `codeLinks` plus the `@concept:<slug>` tags scanned out of the code into `.cache/mapping.json` (via `update-mapping`). A **feature spec** (`features/*.json`) feeds the viewer elsewhere — its `concepts` power the home index row's chips (and a concept detail's "related features"), and its `codePaths` feed the commit gate's drift-follow judgment (author it with `conceptpowers:define-feature`). `render` reads all three (concepts, features, mapping) and rebuilds `manifest.json`.

The graph is data-driven and re-derived on every `render` / `update-mapping`, so it always reflects the current concepts and `@concept` ↔ code links.
